...
public class ColaClientesArray {
    ...
    public void agregarCola (String cliente, String correo, String tipo,String hora,String fecha) throws MessagingException {
        
    if (tipo == "discapacitado"){
        this.discapacitados.agregar(cliente, correo, tipo,hora,fecha);
        this.discapacitadoscomp.agregarC(cliente, correo, tipo,hora,fecha);
    }
    
    else if (tipo == "adulto mayor"){
        
        this.adultomayor.agregar(cliente, correo, tipo,hora,fecha);
        this.adultomayorcomp.agregarC(cliente, correo, tipo,hora,fecha);      
    }
    
    else if (tipo == "embarazada"){
        this.embarazada.agregar(cliente, correo, tipo,hora,fecha);
        this.embarazadacomp.agregarC(cliente, correo, tipo,hora,fecha);
    }
    else if (tipo == "corporativo"){
        this.corporativo.agregar(cliente, correo, tipo,hora,fecha);
        this.corporativocomp.agregarC(cliente, correo, tipo,hora,fecha);
    }   
    
    else if (tipo == "normal"){
        this.normal.agregar(cliente, correo, tipo,hora,fecha);
        this.normalcomp.agregarC(cliente, correo, tipo,hora,fecha);     
    } 
